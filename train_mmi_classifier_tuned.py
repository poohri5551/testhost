...
def evaluate_predictions(y_true, y_pred):
    ...
    acc_pm1 = float(np.mean(np.abs(y_pred - y_true) <= 1))
    return {
        "cv_mae": float(mae),
        "cv_rmse": float(rmse),
...
def main():
    ...
    for name, (model, param_dist) in spaces.items():
        ...
        if result["cv_accuracy_exact"] > best_exact:
            best_exact = result["cv_accuracy_exact"]
            best_name = name
    ...
    report = {
        ...
        "best_cv_accuracy_exact": float(best_exact),
        "cv_strategy": cv_name,
        "all_results": all_results,
        "note": "Tuned classifier with randomized search.",
    }
